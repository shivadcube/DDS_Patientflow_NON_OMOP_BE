...
def job_tracker():
    try:
        ...
        zeppelin_host = os.getenv("ZEPPELIN_HOST", "user")
        mysql_connector, sql_cursor, mysql_status = mysql_connect(host, user, passwd, db, port)
        if not mysql_status:
            sys.exit('mysql connection issue')
        sql_cursor.execute(
            "select notebook_id, analysis, attribute_type, id, paragraph_id from projects_backgroundjobs where status = 'READY' or status = 'RUNNING' ")
        rows = sql_cursor.fetchall()
        if not (rows is None):
            for rec in rows:
                if rec is not None:
                    url = str(zeppelin_host) + "/api/notebook/"
                    resp = requests.get(url + str(rec[0]) + '/paragraph/' + str(rec[4]))
                    ...
                    if status in ["FINISHED", "READY", "RUNNING"]:
                        update_query = "update projects_backgroundjobs set status = '{}', error_message = null, modified_at = '{}' where notebook_id = '{}' and id = {}".format(
                            status, datetime.now(), rec[0], rec[3])
                        ...
                        update_query = "update projects_backgroundjobs set status = '{}', modified_at = '{}', error_message = '{}' where notebook_id = '{}' and id = {}".format(
                            status, datetime.now(), error_msg, rec[0], rec[3])
                        sql_cursor.execute(update_query)
                        mysql_connector.commit()
                        print(sql_cursor.rowcount, "record(s) affected")
                    ...
                    read_query = "select B.project_id as project_id, B.notebook_id as notebook_id , B.status as " \
                                 "status, B.created_at as created_at, B.error_message as error_message, B.modified_at" \
                                 " as modified_at, P.name as project_name, P.created_by as username, U.email as email," \
                                 " B.analysis as analysis_id from {}.projects_backgroundjobs B join projects_projects" \
                                 " P on P.id=B.project_id join auth_user U on U.username=P.created_by where " \
                                 "B.notebook_id='{}' and B.analysis='{}' and B.id = {}".format(database, rec[0], rec[1], rec[3])
                    ...
                    if status in ["FINISHED", "ERROR"]:
                        ...
                        output_data['attribute_type'] = rec[2]
